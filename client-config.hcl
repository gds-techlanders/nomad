...
name = "nomad_client_b"   ## CAN BE HOSTNAME or DIFF NAME, BUT MUST BE UNIQUE IN YOUR CLUSTER 
...
advertise {
  # Defaults to the first private IP address. CHANGE BELOW WITH YOUR PRIVATE IP OF THE YOUR VM
...
client {
  ...
  server_join {
    retry_join = ["provider=aws tag_key=nomad_cluster_id tag_value=sa-east-1"]   //REPLACE THIS KEY AND VALUE TO ANY OTHER VALUE THAT YOU DEFINED ON SERVER MACHINES AND ALSO ADD ROLES FOR CLIENT MACHINE TO PROVIDE PERMISSION TO VIEW SERVERS
  }
